...
export default {
    ...
    actions: {
        ...
        async register({commit}, data) {
            ...
            // await ServicesConst.apiClient.get('/sanctum/csrf-cookie');
            return new Promise((resolve,reject) => {
                ServicesConst.myApiClient.post('/register',data)
                .then((res) => {
                    console.log('res :');
                    console.log(res);
                    resolve(res)
        ...
        async user({commit,rootState}){
            await UserServices.user().then(resp=>{
                ...
                const newUser = {isLoggedIn: true,userId: resp.data.user.id ,userData: {email:resp.data.user.email, userName:resp.data.user.name}};
                ...
                rootState.loggedInUserID = resp.data.user.id;
                rootState.loggedInUser = {email:resp.data.user.email, userName:resp.data.user.name};
                rootState.user = resp.data.user;
